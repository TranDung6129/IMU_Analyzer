...
class ConfigLoader:
    ...
    def _merge_configs(self, config1, config2):
        """
        Hợp nhất hai cấu hình, với config2 được ưu tiên.
        
        Args:
            config1: Cấu hình cơ bản (dict hoặc list)
            config2: Cấu hình được hợp nhất lên trên cơ bản (dict hoặc list)
            
        Returns:
            Cấu hình đã hợp nhất (dict hoặc list)
        """
        # Kiểm tra trường hợp đặc biệt: config2 là list
        if isinstance(config2, list):
            return config2
        
        # Kiểm tra trường hợp đặc biệt: config1 là list 
        if isinstance(config1, list):
            return config2 if config2 is not None else config1
        
        # Kiểm tra trường hợp config1 hoặc config2 không phải là dictionary
        if not isinstance(config1, dict) or not isinstance(config2, dict):
            return config2
        
        # Xử lý trường hợp hai dictionary
        if not config1:
            return config2
        if not config2:
            return config1
    ...
    def _validate(self, config):
        ...
        for section in required_sections:
            if section not in config:
                ...
                # Tạo cấu hình mặc định nếu thiếu
                if section == "logging":
                    config["logging"] = {
                        "level": "INFO",
        ...
        # Xác thực cấu trúc của sensors
        if "sensors" in config:
            sensors_config = config["sensors"]
            if not isinstance(sensors_config, list):
                self.logger.warning("sensors phải là một list")
                config["sensors"] = []
            else:
                for sensor in sensors_config:
                    if not isinstance(sensor, dict):
                        self.logger.warning("Mỗi sensor phải là một dictionary")
                        continue
                    
                    required_sensor_fields = ["id", "type", "connection"]
                    for field in required_sensor_fields:
                        if field not in sensor:
                            self.logger.warning(f"Sensor thiếu trường bắt buộc: {field}")
                            
                    # Xác thực connection
                    if "connection" in sensor:
                        conn = sensor["connection"]
                        if not isinstance(conn, dict):
                            self.logger.warning("connection phải là một dictionary")
                            continue
                        
                        # Xác thực các trường connection dựa trên type
                        conn_type = sensor["type"].lower()
                        if conn_type == "serial":
                            required_conn_fields = ["port", "baudrate"]
                        elif conn_type == "file":
                            required_conn_fields = ["file_path"]
                        elif conn_type == "bluetooth":
                            required_conn_fields = ["device"]
                        else:
                            self.logger.warning(f"Loại sensor không hỗ trợ: {sensor['type']}")
                            continue
                            
                        for field in required_conn_fields:
                            if field not in conn:
                                self.logger.warning(f"Thiếu trường connection bắt buộc: {field}")
